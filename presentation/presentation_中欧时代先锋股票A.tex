...
\begin{document}
	...
	\section[因子计算]{因子月度时间序列计算结果}
	\begin{frame}[t]  % [t] 让内容顶对齐，避免空白
		\frametitle{月度时间序列计算结果比较：Carhart四因子}
		
		\centering
		% 第 1 行
		\includegraphics[width=0.25\linewidth]{image/mkt_1.png}\hfill
		\includegraphics[width=0.25\linewidth]{image/smb_1.png}\hfill
		\includegraphics[width=0.25\linewidth]{image/hml_1.png}\hfill
		\includegraphics[width=0.25\linewidth]{image/umd_1.png}\\[1pt]
		
		% 第 2 行
		\includegraphics[width=0.25\linewidth]{image/mkt_2.png}\hfill
		\includegraphics[width=0.25\linewidth]{image/smb_2.png}\hfill
		\includegraphics[width=0.25\linewidth]{image/hml_2.png}\hfill
		\includegraphics[width=0.25\linewidth]{image/umd_2.png}\\[1pt]
		
		% 第 3 行
		\includegraphics[width=0.25\linewidth]{image/mkt_3.png}\hfill
		\includegraphics[width=0.25\linewidth]{image/smb_3.png}\hfill
		\includegraphics[width=0.25\linewidth]{image/hml_3.png}\hfill
		\includegraphics[width=0.25\linewidth]{image/umd_3.png}\\[1pt]
		
	\end{frame}
	
	
	\begin{frame}{月度时间序列计算结果比较：FIN}
		\footnotesize
		\vspace{-1em}                    % 可微调垂直位置
		\begin{columns}[T,onlytextwidth] % T=顶部对齐
			\begin{column}{.48\textwidth}
				\centering
				\includegraphics[width=\linewidth]{image/fin_1.png}
			\end{column}
			\begin{column}{.48\textwidth}
				\centering
				\includegraphics[width=\linewidth]{image/fin_2.png}
			\end{column}
		\end{columns}
		
		\begin{block}{\textbf{FIN 因子构造}}
			\begin{itemize}
				\[
				\mathrm{CEI}_t = \ln\bigl(\texttt{ME\_t} / \texttt{ME\_t5}) - \ln\bigl(1 + \texttt{Ret5y})
				\]
				\[
				\text{FIN} = \frac{1}{2}(\text{S/L}_{\text{CEI}} + \text{B/L}_{\text{CEI}}) - \frac{1}{2}(\text{S/H}_{\text{CEI}} + \text{B/H}_{\text{CEI}})
				\]
	...
	\begin{frame}{月度时间序列计算结果比较：FIN}
		...
		\begin{block}{\textbf{FIN 因子构造}}
			\begin{itemize}
				\item \textbf{经济含义：}
				CEI 剔除股价自然上涨后，捕捉「公司是否靠发新股/配股做大规模」；  
				值越高 → 外部融资越激进 → FIN 因子做空该组。
				\item \textbf{行为学依据：}
				投资者系统性高估外部融资企业的未来盈利增速（\emph{over-extrapolation}），导致其股价被暂时推高；随后盈利兑现不足，价格回落——FIN 因子正是做空这类“融资亢奋”错误定价。
			\end{itemize}
		\end{block}
	\end{frame}
	
	\begin{frame}{因子的历史累计回报：计算值}
		\footnotesize
		\vspace{-1em}                    % 可微调垂直位置
		\begin{columns}[T,onlytextwidth] % T=顶部对齐
			\begin{column}{1\textwidth}
				\centering
				\includegraphics[width=\linewidth]{image/CR_min.png}
			\end{column}
			
		\end{columns}
		\begin{block}{\textbf{回报表现}}
			\begin{itemize}
				\item \textbf{MKT：}
				年化收益=0.1182，年化波动=0.1794，夏普比率=0.6593；表现最佳，具有最高的年化收益和夏普比率
				\item \textbf{SMB：}
				年化收益=0.0606，年化波动=0.1435，夏普比率=0.4220；小市值股票在这段时间内表现较好
				\item \textbf{HML：}
				年化收益=-0.0117，年化波动=0.1475，夏普比率=-0.0792；价值股在这段时间内表现不佳
	...
	\begin{frame}{因子的历史累计回报：计算值}
		...
		\begin{columns}[T,onlytextwidth] % T=顶部对齐
			\begin{column}{0.6\textwidth}
				\centering
				\includegraphics[width=\linewidth]{image/CR.png}
			\end{column}
			\begin{column}{0.33\textwidth}
				\centering
				\includegraphics[width=\linewidth]{image/corr.png}        
			\end{column}
		...
		\begin{block}{\textbf{回报表现}}
			\begin{itemize}
				\item \textbf{FIN：}
				年化收益=0.0150，年化波动=0.1108，夏普比率=0.1351；远离激进的外部融资股票可以带来正收益
				\item \textbf{UMD：}
				年化收益=-0.0083，年化波动=0.1289，夏普比率=-0.0646；动量策略在这段时间内未能提供正的超额收益
				\item \textbf{联合分析}
			\end{itemize}
		\end{block}
	\end{frame}
	
	\begin{frame}{因子的历史累计回报：课堂文件}
		\footnotesize
		\vspace{-1em}                    % 可微调垂直位置
		
		\begin{column}{1\textwidth}
			\centering
			\includegraphics[width=\linewidth]{image/CRR.png}
		\end{column}
		...
		\begin{columns}
			\begin{column}{1\textwidth}
				\begin{block}{小组成员分工}
					\begin{itemize}
						\item 董安愉: 
						\item 李想: 完成因子计算及分析（对应作业文件1题）；后续步骤的基础数据准备下载
